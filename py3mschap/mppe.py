...
def verify_radius_request(ms_chap_response, authenticator_challenge, username, userpwd, secret, authenticator):
    ...
    nt_resp = mschap.generate_nt_response_mschap2(authenticator_challenge, peer_challenge, username, userpwd, )
    if nt_resp == nt_response:
        auth_resp = mschap.generate_authenticator_response(userpwd, nt_response, peer_challenge,
            authenticator_challenge, username)
        ...
        mppeSendKey, mppeRecvKey = mppe_chap2_gen_keys(userpwd, nt_response)
        send_key, recv_key = gen_radius_encrypt_keys(mppeSendKey, mppeRecvKey, secret, authenticator)
